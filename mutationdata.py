def gene_network(sorteddatafile): 
	...
	under60 = len(data[0:3])
	over60 = len(over60_data)
	...
	topgene_compare_60 = numpy.zeros([10,10])
	...
	for g in range(gene_length): 
		total = 0 
		total_60 = 0 
		for l in range(under60):
			total = total + data[l][2][g]
		total_and_genes.append([total,cosmicset[g]])
		for m in range(over60): 
			total_60 = total_60 + over60_data[m][2][g]
		total_and_genes_60.append([total_60,cosmicset[g]])	
			
	total_and_genes.sort()
	total_and_genes.reverse()
	total_and_genes_60.sort()
	total_and_genes_60.reverse()
	print 'total array: ', total_and_genes, '\n', '60 total array: ', total_and_genes_60
	toptotal_w_genes = total_and_genes[:10]
	toptotal_w_genes_60 = total_and_genes_60[:10]
	print 'top totals: ', toptotal_w_genes, '\n', '60 top totals: ', toptotal_w_genes_60
	for genename in toptotal_w_genes:
		toptengenes.append(genename[1])
	for genes in toptotal_w_genes_60:
		toptengenes_60.append(genes[1])
	print 'top ten genes: ', toptengenes, '\n', '60 top ten genes: ', toptengenes_60
	for item in toptengenes:
		rowgenename.write(j,item)
		sheet11.write(k,0,item)
		j = j + 1
		k = k + 1
	for point in toptengenes_60: 
	...
	for sample in somaticgene_d: 
		for genetics in range(len(toptengenes_60)): 
			if toptengenes_60[genetics] in somaticgene_d[sample] and sample[1] >= 60: 
				for f in range(len(toptengenes_60)): 
					if toptengenes_60[f] in somaticgene_d[sample]: 
						topgene_compare_60[genetics][f] = topgene_compare_60[genetics][f] + 1
						
	print '10 by 10 matrix over 60  ', topgene_compare_60
	print 'top ten genes over 60: ' , toptengenes_60
	print 'toptotals_60:', toptotal_w_genes_60				
	
	for row in range(topgene_compare_60.shape[0]): 
		for column in range(topgene_compare_60.shape[1]): 
			sheet11.write(row+16,column+1,topgene_compare_60[row][column])
...
def total_missensemutation_sample(sorteddatafile):
	...
	a = 0
	for i in data:  
		if i[3] == "Missense_Mutation": #only takes data with a mutated gene
...
def missense_vs_age(datasorted_BRCA):
	datasorted_BRCA = open(datasorted_BRCA, 'r')
	data = datasorted_BRCA.readlines();
	age_dictionary = {}
	ages = [30,40,50,60,70,80] 
	from xlrd import open_workbook
	from xlutils.copy import copy
...
def age_vs_frequency(datafile): 
	...
	print 'graph is of important genes with frequency >= 7'
	...
	print 'graph is of important genes with match > 7'
